...
def parse_relevance_strings(strings):
    ...
    for i in range(len(strings)):
        str_parts = []
        str_parts = re.split("\s",strings[i])
        str_parts = [token for token in str_parts if token]
        if not str_parts:
            continue
        else:    
            result[int(str_parts[0])] = list(map(int,str_parts[1:])) # splice array from second element onwards
...
def parse_query_strings(strings):
    ...
    str_final = re.split(r'\*FIND\s*\d*|\*STOP',total_string)
    str_final1 = list(filter(lambda name: name.strip(), str_final))
    for k in range(len(str_final1)):
        string_val = re.sub(r"\n"," ",str_final1[k].strip()+" ")
        res[k+1] = re.sub(r"  "," ",string_val) 
    return res
    pass
...
def parse_document_strings(strings):
    ...
    str_final = []
    total_string = " ".join(strings)
    str_final = re.split(r'\*TEXT\s*\d*\s*\d+/\d+/\d+\s*PAGE\s*\d{3}|\*TEXT\s*\d*\s*\d+/\d+\s*\d+\s*PAGE\s*\d{3}|\*STOP',total_string)
    
    str_final1 = list(filter(lambda name: name.strip(), str_final))
    for k in range(len(str_final1)):
        string_val = re.sub(r"\n"," ",str_final1[k].strip()+" ")
        string_val = re.sub(r"  "," ",string_val)
        str_tokens.append(string_val.strip()+" ")
